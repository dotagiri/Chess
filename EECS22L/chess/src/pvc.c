...
int playerVScomputer(int menu_returned_value)
{
	...
	//int level = 0;		//level determines easy, medium or hard		- line commented out because not incorporating levels of difficulty
	...
	//comments in lines 48 - 79 will be included if we have multiple difficulty levels
	...
	*/
	...
	if (first == 1)
	{
		...
		if (i % 2 == 0) /* player turn */
		{
			...
			chessOutput(board[final_y][final_x], colIndex2ColLetter(initial_x), arrayRow2ChessRow(initial_y), colIndex2ColLetter(final_x), arrayRow2ChessRow(final_y));
		...
		//comments in lines 150 - 178 will be included if we have multiple difficulty levels
		/*
	...
	if (first == 2)
	{	
		cpuColor = 1; 	//for calling ai function, shows that CPU is white (first)
		i = i + 1;		// i starts off odd and computer starts first
		
		while (a == 0) 		// a = 0 -> no checkmate
		{
		
		//comments in lines 202 - 230 will be included if we have multiple difficulty levels
